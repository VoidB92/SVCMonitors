...
static unsigned long long g_events_total = 0;
...
static void get_current_comm(char *buf, int size)
{
    ...
    __asm__ volatile("mrs %0, tpidr_el1" : "=r"(current_task));
    if (!current_task) {
        buf[0] = '?'; buf[1] = '\0';
        return;
    }
...
static void capture_caller(struct svc_event *ev, void *fargs_raw)
{
    ...
    if (has_syscall_wrapper) {
        ...
        struct pt_regs *regs = (struct pt_regs *)fa->arg0;
        if (regs) {
            ev->caller_pc = (unsigned long long)regs->pc;
            ev->caller_lr = (unsigned long long)regs->regs[30];
        }
    } else {
        hook_fargs6_t *fa = (hook_fargs6_t *)fargs_raw;
        ev->caller_lr = (unsigned long long)fa->arg6;
    }
...
static void before_syscall_from_hook(void *fargs_raw, int nr)
{
    ...
    if (!bitmap_test(nr)) { g_in_hook = 0; return; }
...
static void remove_hook(int nr)
{
    ...
    for (i = 0; i < g_hook_count; i++) {
        if (g_hooked_nrs[i] == nr) {
            g_hooked_nrs[i] = g_hooked_nrs[g_hook_count - 1];
            g_hook_count--;
            break;
        }
    }
...
{
    while (g_hook_count > 0) {
        remove_hook(g_hooked_nrs[0]);
    }
}
...
static int format_so_offset(char *dst, int dstlen, const char *path, unsigned long long offset)
{
    const char *base = path_basename(path);
    if (!dst || dstlen <= 0) return 0;
    if (!base[0]) return __builtin_snprintf(dst, dstlen, "unknown+0x%llx", offset);
    if (base[0] == '0' && base[1] == 'x') return __builtin_snprintf(dst, dstlen, "%s", base);
    return __builtin_snprintf(dst, dstlen, "%s+0x%llx", base, offset);
}

static void output_simple_result(int ok, const char *msg, const char *error)
{
    char msg_esc[256];
    char err_esc[256];
    if (!msg) msg = "";
    ...
    json_escape(error, err_esc, sizeof(err_esc));
    g_outpos = __builtin_snprintf(g_outbuf, OUTPUT_BUF_SIZE,
        "{\"ok\":%s,\"msg\":\"%s\",\"error\":\"%s\"}\n",
        ok ? "true" : "false", msg_esc, err_esc);
}

static int str_eq(const char *a, const char *b);
static int simple_atoi(const char *s);

static int is_tier2_nr(int nr)
{
    int i;
    for (i = 0; i < g_tier2_count; i++) {
        if (g_tier2_nrs[i] == nr) return 1;
    }
    return 0;
}

static void output_status_v8(void)
{
    int i;
    int tier2 = 0;
    g_outpos = 0;
    ...
    g_outpos += __builtin_snprintf(g_outbuf + g_outpos, OUTPUT_BUF_SIZE - g_outpos,
        "{\"ok\":true,\"version\":\"9.0.0\",\"enabled\":%s,"
        "\"target_uid\":%d,\"hooks_installed\":%d,\"nrs_logging\":%d,"
        "\"events_total\":%llu,\"events_buffered\":%d,"
        "\"logging_nrs\":[",
        g_running ? "true" : "false",
        g_target_uid ? (int)g_target_uid : -1,
        g_hook_count, g_hook_count,
        g_events_total, g_ev_count);
    ...
    for (i = 0; i < g_hook_count && g_outpos < OUTPUT_BUF_SIZE - 64; i++) {
        ...
        g_outpos += __builtin_snprintf(g_outbuf + g_outpos, OUTPUT_BUF_SIZE - g_outpos, "%d", g_hooked_nrs[i]);
        if (is_tier2_nr(g_hooked_nrs[i])) tier2 = 1;
    }

    g_outpos += __builtin_snprintf(g_outbuf + g_outpos, OUTPUT_BUF_SIZE - g_outpos,
        "],\"hooks\":[");

    for (i = 0; i < g_hook_count && g_outpos < OUTPUT_BUF_SIZE - 128; i++) {
        if (i > 0) g_outpos += __builtin_snprintf(g_outbuf + g_outpos, OUTPUT_BUF_SIZE - g_outpos, ",");
        g_outpos += __builtin_snprintf(g_outbuf + g_outpos, OUTPUT_BUF_SIZE - g_outpos,
            "{\"nr\":%d,\"name\":\"%s\",\"method\":\"hook_syscalln\"}",
            g_hooked_nrs[i], get_syscall_name(g_hooked_nrs[i]));
    }
...
static int parse_onoff(const char *s)
{
    ...
    if (s[0] == '1') return 1;
    if (s[0] == '0') return 0;
    if ((s[0] == 'o' || s[0] == 'O') && (s[1] == 'n' || s[1] == 'N') && s[2] == '\0') return 1;
    if ((s[0] == 'o' || s[0] == 'O') && (s[1] == 'f' || s[1] == 'F') && (s[2] == 'f' || s[2] == 'F') && s[3] == '\0') return 0;
    return simple_atoi(s) != 0;
}

static void drain_events_v8(int max)
{
    struct svc_event ev;
    int drained = 0;
    char comm_esc[64];
    ...
    g_outpos = 0;
    g_outpos += __builtin_snprintf(g_outbuf + g_outpos, OUTPUT_BUF_SIZE - g_outpos,
        "{\"ok\":true,\"count\":0,\"total\":%llu,\"events\":[", g_events_total);

    while (drained < max && pop_event(&ev) && g_outpos < OUTPUT_BUF_SIZE - 2048) {
        if (drained > 0) g_outpos += __builtin_snprintf(g_outbuf + g_outpos, OUTPUT_BUF_SIZE - g_outpos, ",");
        json_escape(ev.comm, comm_esc, sizeof(comm_esc));
        json_escape(ev.arg_detail, desc_esc, sizeof(desc_esc));
        g_outpos += __builtin_snprintf(g_outbuf + g_outpos, OUTPUT_BUF_SIZE - g_outpos,
            "{\"seq\":%u,\"nr\":%d,\"name\":\"%s\",\"pid\":%d,\"uid\":%u,"
            "\"comm\":\"%s\",\"pc\":%llu,\"caller\":%llu,\"clone_fn\":0,"
            "\"a0\":%llu,\"a1\":%llu,\"a2\":%llu,\"a3\":%llu,\"a4\":%llu,\"a5\":%llu,"
            "\"desc\":\"%s\"}",
...
static void apply_preset(const char *name)
{
    ...
    if (!name) {
        g_outpos = __builtin_snprintf(g_outbuf, OUTPUT_BUF_SIZE,
            "{\"ok\":false,\"error\":\"no preset name\"}\n");
        return;
    }

    remove_all_hooks();

    if (str_eq(name, "re_basic")) {
        static const int arr[] = {56,48,78,221,222,226,198,203,117,167,29,134,279,280};
        nrs = arr;
        cnt = (int)(sizeof(arr) / sizeof(arr[0]));
    } else if (str_eq(name, "re_full")) {
        static const int arr[] = {56,48,35,78,79,63,64,221,281,220,93,94,222,226,215,198,200,203,206,207,117,167,29,25,129,131,134,135,277,280,279,146,144,147,105,273,97,268,261,291};
        nrs = arr;
        cnt = (int)(sizeof(arr) / sizeof(arr[0]));
    } else if (str_eq(name, "file")) {
        static const int arr[] = {56,57,48,35,78,79,80,61,63,64,62,34,38,276,53,54,291,43,27,5,8};
        nrs = arr;
        cnt = (int)(sizeof(arr) / sizeof(arr[0]));
    } else if (str_eq(name, "net")) {
        static const int arr[] = {198,200,201,202,203,206,207,208,209,210,211,212,242};
        nrs = arr;
        cnt = (int)(sizeof(arr) / sizeof(arr[0]));
    } else if (str_eq(name, "proc")) {
        static const int arr[] = {221,281,220,93,94,95,260,117,129,130,131,270,271};
        nrs = arr;
        cnt = (int)(sizeof(arr) / sizeof(arr[0]));
    } else if (str_eq(name, "mem")) {
        static const int arr[] = {222,226,215,214,233,279,270,271};
        nrs = arr;
        cnt = (int)(sizeof(arr) / sizeof(arr[0]));
    } else if (str_eq(name, "security")) {
        static const int arr[] = {117,277,280,146,144,147,149,105,273,106,97,268,91,167,134};
        nrs = arr;
        cnt = (int)(sizeof(arr) / sizeof(arr[0]));
    } else if (str_eq(name, "tier1")) {
        install_tier1();
    } else if (str_eq(name, "tier2")) {
        install_tier2();
    ...
    if (nrs && cnt > 0) {
        for (i = 0; i < cnt; i++) install_hook(nrs[i]);
    }

    g_outpos = __builtin_snprintf(g_outbuf, OUTPUT_BUF_SIZE,
        "{\"ok\":true,\"preset\":\"%s\",\"hooks_installed\":%d,\"nrs_logging\":%d}\n",
        name, g_hook_count, g_hook_count);
}
...
static void ctl0_dispatch(const char *cmd, char *outbuf, int outlen)
{
    ...
    if (str_eq(cmd, "status")) {
    ...
    else if (str_eq(cmd, "enable")) {
        g_running = 1;
        output_simple_result(1, "enabled", "");
    }
    else if (str_eq(cmd, "disable")) {
        g_running = 0;
        output_simple_result(1, "disabled", "");
    }
    else if (cmd[0]=='d' && cmd[1]=='r' && cmd[2]=='a' && cmd[3]=='i' && cmd[4]=='n' && (cmd[5]=='\0' || cmd[5]==' ')) {
        int max = 100;
        if (cmd[5] == ' ') max = simple_atoi(cmd + 6);
        drain_events_v8(max);
    }
        ...
        g_ev_dropped = 0;
        g_seq = 0;
        output_simple_result(1, "cleared", "");
    }
        ...
        g_outpos = __builtin_snprintf(g_outbuf, OUTPUT_BUF_SIZE,
            "{\"ok\":true,\"target_comm\":\"%s\"}\n", g_target_comm);
    }
    else if (cmd[0]=='p' && cmd[1]=='r' && cmd[2]=='e' && cmd[3]=='s' && cmd[4]=='e' && cmd[5]=='t' && cmd[6]==' ') {
        apply_preset(cmd + 7);
    }
    else if (cmd[0]=='t' && cmd[1]=='i' && cmd[2]=='e' && cmd[3]=='r' && cmd[4]=='2' && cmd[5]==' ') {
        int i;
        int en = parse_onoff(cmd + 6);
        if (en) {
            install_tier2();
            output_simple_result(1, "tier2 enabled", "");
        } else {
            for (i = 0; i < g_tier2_count; i++) remove_hook(g_tier2_nrs[i]);
            output_simple_result(1, "tier2 disabled", "");
        }
    }
    else if (cmd[0]=='e' && cmd[1]=='n' && cmd[2]=='a' && cmd[3]=='b' && cmd[4]=='l' && cmd[5]=='e' && cmd[6]=='_' && cmd[7]=='n' && cmd[8]=='r' && cmd[9]==' ') {
        int nr = simple_atoi(cmd + 10);
        install_hook(nr);
        output_simple_result(1, "nr enabled", "");
    }
    else if (cmd[0]=='d' && cmd[1]=='i' && cmd[2]=='s' && cmd[3]=='a' && cmd[4]=='b' && cmd[5]=='l' && cmd[6]=='e' && cmd[7]=='_' && cmd[8]=='n' && cmd[9]=='r' && cmd[10]==' ') {
        int nr = simple_atoi(cmd + 11);
        remove_hook(nr);
        output_simple_result(1, "nr disabled", "");
    }
    else if (cmd[0]=='s' && cmd[1]=='e' && cmd[2]=='t' && cmd[3]=='_' && cmd[4]=='n' && cmd[5]=='r' && cmd[6]=='s' && cmd[7]==' ') {
        const char *p = cmd + 8;
        remove_all_hooks();
        while (*p) {
            while (*p == ' ' || *p == '\t' || *p == ',') p++;
            if (!*p) break;
            install_hook(simple_atoi(p));
            while (*p && *p != ' ' && *p != '\t' && *p != ',') p++;
        }
        output_simple_result(1, "nrs set", "");
    }
    else if (str_eq(cmd, "enable_all")) {
        remove_all_hooks();
        install_all();
        output_simple_result(1, "all enabled", "");
    }
    else if (str_eq(cmd, "disable_all")) {
        remove_all_hooks();
        output_simple_result(1, "all disabled", "");
    }
